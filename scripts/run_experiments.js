...
const fitnessFns = [
  ...
  'health-damage-salvage',
  'health-damage-explore',
  'health-damage-time-salvage-explore',
];
const runsCount = 200;
const weights = { h: 1.0, d: 1.0, k: 0.5, t: 0.1, s: 0.1, e: 0.05 };
...
for (const teamSize of teamSizes) {
  for (const duration of durations) {
    for (const fitnessFn of fitnessFns) {
      const runId = `${teamSize}v${teamSize}-${duration}s-${fitnessFn}`;
      const args = [
        'train',
      ...
      // add salvage/explore weights if applicable
      if (fitnessFn.includes('salvage')) {
        args.push('--w-salvage', String(weights.s));
      }
      if (fitnessFn.includes('explore')) {
        args.push('--w-explore', String(weights.e));
      }
